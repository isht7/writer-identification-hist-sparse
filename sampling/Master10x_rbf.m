...
%    alpha_tr=abs(alpha1); alpha_te=abs(alpha2);
 alpha_tr = alpha_tr_cell{instance_no}   ;
  alpha_te = alpha_te_cell{instance_no}   ;
    ...
    for ks_iter = 1:10
        for bc_iter = 1:10
            disp('SVM');
            [ AccuracyS,Accuracy3,Accuracy5,acc ] = svm4line_rbf(start_folder, end_folder,W1_4index,W5_8index,alpha_tr,alpha_te,ks,bc);
%            pack('pack_mem.mat')
            gridX = [gridX,acc];
	    gridX_master{instance_no} = gridX;
	    save('gridX_master_rbf_kernel_S.mat','gridX_master')
            ks = 10*ks; bc = 10*bc;
        end
    end
    gridX_master{instance_no} = gridX;
