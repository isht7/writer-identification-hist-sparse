function strokes4 = Sstrokes2_swap(strokes,count)
%% 
...
% Segmenting the sub strokes by cutting at the minima(local maxima's because the image is inverted)
count2 = 1;        
...
if length(A)==2
    if length(str(:,1))>5
    strokes3{count2} = str(:,1:2);
    count2 = count2+1;
    end
else    
    for j = 1:length(A)-1    
        k = str(A(j)+1:A(j+1),1:2);
        if length(k(:,1))>5
        strokes3{count2} = k;
        count2 = count2+1;
        end
